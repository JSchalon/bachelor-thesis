<template>
  ...
  <g v-if="signData.toe >= 1 && signData.toe <=5" :transform="signData.limb ? '' : 'translate(7.5,0)'">
    <g v-if="signData.digit >= 1 && signData.digit < 4">
      <circle fill="black" :cx="18.5 - (index * 4)" :cy="10 + (signData.toe - 1) * 5" r="2" :key="index" v-for="index of (signData.digit)"/>
    </g>
    <circle v-if="signData.digit && signData.digit == 4" fill="black" :cx="18.5 - 3 * 4" :cy="10 + (signData.toe - 1) * 5" r="2" />
  </g>
</template>
